...
# Add padding token to tokenizer (GPT-2 doesn't have a padding token by default)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(train_dataset)):
    ...
    for epoch in range(3):
        ...
        for batch in train_dataloader:
            ...
            # Forward pass for fine and coarse annotations
            fine_outputs = model(input_ids=fine_input_ids, attention_mask=fine_attention_mask).last_hidden_state.mean(
                dim=1)
            coarse_output = model(input_ids=coarse_input_ids).last_hidden_state.mean(dim=1)
