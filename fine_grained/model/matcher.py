...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self,  cost_class: float = 1, cost_span: float = 1, cost_giou: float = 1,
                 span_loss_type: str = "l1", max_v_l: int = 75, multi_clip=False):
        """Creates the matcher
    ...
    def forward(self, outputs, targets):
        ...
        if self.multi_clip:
            normalized_moment = torch.cat([v["moments"] for v in targets["norm_moment"]])
            tgt_spans = torch.cat([v["spans"] for v in targets["norm_span"]])
        else:
            normalized_moment = targets["norm_moment"]
        ...
        if self.span_loss_type == "l1":
            ...
            cost_giou = - generalized_temporal_iou(span_cxw_to_xx(out_spans), normalized_moment)
        else:
        ...
        if self.multi_clip:
            sizes = [len(v["spans"]) for v in targets["norm_span"]]
